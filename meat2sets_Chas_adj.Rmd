---
title: "R and Matlab (both NPSB and OSPB 'meat'=GE) Chastity Adjusted (Irina) DEGs SEGs"
...
- read Chasity adjusted (based on OPSB, Irina) unique set, Nav2 from 25 April
...
peems + facet_wrap( ~ set)+ggtitle(" sets with OPSB")
...
```
- save/write standard Matlab format (no 'set' column )
```{r}
...
- split due to set, new GE from NPSB
```{r}
...
pecenms + facet_wrap( ~ set) +ggtitle(" with new PSB GE data")
